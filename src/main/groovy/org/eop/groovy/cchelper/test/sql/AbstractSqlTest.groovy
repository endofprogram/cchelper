...
abstract class AbstractSqlTest extends AbstractCommonTest {
	...
	String getBusiSystemSql() {
		"""\
/* 业务系统 */
INSERT INTO `${db.name}`.`bs_busi_system` (`BIZ_SYS_ID`, `BIZ_SYS_NM`, `BIZ_SYS_DESC`, `CRT_TIME`, `OP_TIME`, `PROV_ID`, `BIZ_SYS_CODE`)
...
"""
	...
	String getChannelSql() {
		"""\
/* 渠道 */
INSERT INTO `$db.name`.`bs_interface_channel` (`CHNL_ID`, `SYS_ID`, `CHNL_JNUP_MODE_NM`, `CHNL_ADDR`, `CRT_TIME`, `OP_TIME`, `SYS_STS_CD`, `CHNL_NM`)
...
'''
		staticConfig.each({
			list <<	"""\
INSERT INTO `$db.name`.`bs_static_config` (`CONFIG_ID`, `CONFIG_TYPE_CD`, `FST_CONFIG_ITEM_NM`, `SECD_CONFIG_ITEM_NM`, `CONFIG_ITEM_CHAR_ATTR_VAL`, `RMK`, `STS_CD`, `CRT_TIME`, `MODF_TIME`)
...
'''
		staticConfigTokenInterfaceIds.eachWithIndex{it, index ->
			list <<	"""\
update `$db.name`.`bs_static_config` c set c.`CONFIG_ITEM_CHAR_ATTR_VAL` = ${ids[index]}
...
"""
	...
	String getInInterfaceSql() {
		"""\
/* 接入接口 */
INSERT INTO `$db.name`.`bs_interface` (`INTF_ID`, `ROUTE_RULE_ID`, `BIZ_SYS_ID`, `INTF_NM`, `INTF_CLA_NM`, `OUT_SYSTEM_TYPE`, `INTF_TYPE_CD`, `BACK_CHNL_ID`, `INTF_PATH`, `INTF_BIZ_CODE`, `RSTFL_REQST_MTHD_NM`, `MSG_FMT_TYPE_CD`, `AGRMT_ENT_ID`, `INTF_DESC`, `INTF_BIZ_TYPE_ID`, `EFF_TIME`, `INVLD_TIME`, `SYS_STS_CD`, `CRT_TIME`, `OP_TIME`)
...
"""
	...
	String getInterfaceMappingSql() {
		"""\
/* 接口映射 */
INSERT INTO `$db.name`.`bs_interface_mapping` (`INTF_ID`, `MAPNG_INTF_ID`, `PRI_LVL_SEQNO`, `CRT_TIME`, `CHNL_ID`, `DEPEND_INTF_ID`)
...
'''
		inInterfaceParams.each({
			list << """\
INSERT INTO `$db.name`.`bs_interface_param` (`PARA_ID`, `ENCR_STG_ID`, `INTF_ID`, `SRV_PARA_TYPE_CD`, `PARA_NM`, `DATA_TYPE_NM`, `DATA_LEN_VAL`, `NULL_FLAG`, `ARGE_SEQNO`, `CRT_TIME`, `OP_TIME`)
...
'''
		interfaceParamsMapping.each({
			list << """\
INSERT INTO `$db.name`.`bs_interface_param_mapping` (`SRC_PARA_ID`, `MAPNG_PARA_ID`, `PARA_TRSFM_SCPT_CNTT`, `PARA_TRSFM_ACHV_CLA_NM`, `CRT_TIME`, `OP_TIME`)
...
'''
		outInterfaceParamsStatic.each({
			list << """\
INSERT INTO `$db.name`.`bs_interface_param_static` (`PARA_ID`, `MAPNG_INTF_ID`, `PARA_KEY`, `PARA_VALUE`, `ARGE_SEQNO`, `SYS_STS_CD`, `CRT_TIME`, `OP_TIME`)
...
'''
		dependentInterfaces.each{
			list << """\
update `$db.name`.`bs_interface_mapping` m set m.`DEPEND_INTF_ID` = case when length(trim(m.`DEPEND_INTF_ID`)) < 1 then ${getDependentInterfaceId(it.desc)} else concat(m.`DEPEND_INTF_ID`,',',${getDependentInterfaceId(it.desc)}) end
...
"""
	...
	String getChannelAuthSql() {
		"""\
/* 渠道权限 */
INSERT INTO `$db.name`.`bs_in_chnannel_auth` (`JNUP_CHNL_PRIV_ID`, `JNUP_CHNL_ID`, `JNUP_INTF_ID`, `SYS_STS_CD`, `CRT_TIME`)
select (select max(cast(a.jnup_chnl_priv_id as signed)) + 1 from bs_in_chnannel_auth a), (select c.chnl_id from bs_interface_channel c where c.sys_id = (select b.biz_sys_id from bs_busi_system b where b.biz_sys_code = 'CRMPF')), $inInterfaceId, '1', now();
...
"""
	...
	String getEncryptRelSql() {
		"""\
/* 接口加密引用 */
INSERT INTO `$db.name`.`bs_interfac_encrypt_tactic_rel` (`INTF_ENCR_ID`, `INTF_ID`, `ENCR_STG_ID`, `ENCR_TYPE_CD`, `CRT_TIME`, `OP_TIME`)
...
"""
	...
	String getSecretKeySql() {
		"""\
/* 密钥 */
INSERT INTO `$db.name`.`bs_secret_key` (`ENCR_CRKEY_ID`, `ENCR_STG_ID`, `ENCR_CRKEY`, `DECR_CRKEY`, `CRKEY_VERNO`, `EFF_TIME`, `INVLD_TIME`, `CRT_DATE`, `SYS_STS_CD`, `OP_TIME`, `INTF_ID`)
...
"""
	...
	String getCallbackMappingSql() {
		"""\
/* 回调映射 */
INSERT INTO `$db.name`.`bs_callback_uniform_mapping` (`ID`, `BIZ_SYS_ID`, `TRSTCH_INTF_ID`, `INTF_CALLBCK_ADDR`, `CALLBCK_MSG_PRS_CLA_NM`, `SYS_STS_CD`, `EFF_TIME`, `CRT_TIME`)
...
"""
	...
	String getCallbackSql() {
		"""\
/* 回调 */
INSERT INTO `$db.name`.`bs_callback_uniform` (`ID`, `JNUP_CHNL_ID`, `JNUP_INTF_ID`, `INTF_SRV_CODE`, `REST_METHOD_TYPE`, `URL_ADDR`, `SYS_STS_CD`, `EFF_TIME`, `CRT_TIME`)
...
"""
	...
	String getCrmpfBusiSystemId() {
		"""(select b.biz_sys_id from bs_busi_system b where b.biz_sys_code = 'CRMPF')"""
	}
