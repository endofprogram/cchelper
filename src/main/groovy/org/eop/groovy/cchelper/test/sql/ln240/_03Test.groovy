...
class _03Test extends AbstractSqlTest {
	...
	@Override
	Map getSqlFile() {
		[dir:'E:\\yaxin\\需求\\辽宁流量统付\\数据库\\测试环境\\', name:'03充值结果查询']
	}
	...
	@Override
	Map getInInterface() {
		[route:'', name:'在线统一接口_业务_流量充值订单状态查询', clsnm:'', type:'', path:'', bizcode:'', method:'GET', fmttype:'json', entid:'', desc:'在线统一接口_业务_流量充值订单状态查询', biztype:'001']
	}
	
	@Override
	Map getOutInterface() {
		[route:'', name:'辽宁流量统付_充值结果查询', clsnm:'lnFlowOrderQueryService', type:'', path:'/oppf', bizcode:'', method:'POST', fmttype:'json', entid:'10', desc:'辽宁流量统付_充值结果查询(20170619_out)', biztype:'001']
	}
	...
	List getOutInterfaceParamsStatic() {
		...
		list << [name:'State', value:'2', seqno:'06']
		list << [name:'Begin', value:'-1', seqno:'06']
		list << [name:'End', value:'-1', seqno:'06']
		list
	}
